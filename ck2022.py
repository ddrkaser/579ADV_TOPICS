...
ck_2022.columns
cols = [ 'game_date',	'release_speed',	'release_pos_x',	'release_pos_z',	'description',	'zone',	'stand',	'balls',	'strikes',	'pfx_x'	, 'pfx_z',	'plate_x','plate_z', 'vx0',	'vy0',	'vz0', 'ax',	'ay',	'az',	'effective_speed',	'release_spin_rate',	'release_extension',	'at_bat_number',	'pitch_name',	'if_fielding_alignment',	'of_fielding_alignment',	'spin_axis']
...
dot_size = (df['release_speed']*0.1)**3
fig, ax = plt.subplots(figsize=(15,10))
scatter = ax.scatter(x=df['release_pos_x'], y=df['release_pos_z'], s=dot_size,c=pitch_type_encoded, alpha=0.3)
...
legend2 = ax.legend(handles, labels, loc="lower left", title="Speed")
plt.show()
...
#
ck_2022 = ck_2022[cols].dropna().loc[::-1].reset_index(drop=True)
df_reverseed = ck_2022[attributes_PCA]
...
ck_2022['game_date'].value_counts().mean()
m = 84
...
rect = Rectangle((discord_idx, -200), m, 600, facecolor='lightgrey')
...
neigh.fit(pca_data,pitch_type_encoded.flatten())
A = neigh.kneighbors_graph(pca_data)
